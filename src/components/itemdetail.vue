				...
				<div class="item-box">
					<div class="gallery-wrapper">
						<div class="gallery">
							<div class="thumbnail">
								<ul>
									<li :class="{'on':index==imgindex }"
                                        v-for="img, index in detaildata.ali_images"
                                        @click="tabimgindex(index)"
						...
						<div class="sku-dynamic-params-panel">
							<div class="sku-dynamic-params clear">
								...
								<ul class="params-colors">
									<li :class="{'cur':sku.id == $route.query.itemid}" v-for="sku, index in detaildata.sku_list">
										<!-- <a><img :src="`http://img01.smartisanos.cn/${sku.image}20X20.jpg`"></a> -->
                <router-link :to="{name: 'item', query:{itemid: sku.id} }" >
                    <img :src="`http://img01.smartisanos.cn/${sku.image}20X20.jpg`">
                </router-link>            
									</li>
								</ul>
							</div>
                    ...
                    <span class="up" :class="{'up-disabled': count>=detaildata.limit_num}" @click="addcount">+</span>
									</div>
								</div>
							</div>
						</div>
						<div class="sku-status">
							<div class="cart-operation-wrapper clearfix">
								<span class="blue-title-btn js-add-cart" @click="addgoods()">
                                    <a>加入购物车</a>
                                </span>
...
<script>
    ...
    export default {
        ...
        computed:{
            detaildata(){
                let data = this.newItemsData.filter(data =>{
                    return Number(data.sku_id) === Number(this.itemid)
                })[0]
                return data
                // console.log(data)
            }
        ...
        methods: {
            ...
            addcount(){
                this.count ++;
                if (this.count >= this.detaildata.limit_num) {
                    this.count = this.detaildata.limit_num
                }
            },
            subcount(){
                this.count --;
            ...
            addgoods(item){
                let data = {'item': this.detaildata, 'num': this.count}
