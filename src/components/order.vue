...
<div class="store-content">
    <div class="cart-box">
        ...
        <div class="cart-inner">
            <div class="empty-label" v-show="totalcount<1">
                <h3>您的购物车中还没有商品</h3>
                <router-link class="link" :to="{name:'index'}" >现在选购</router-link>
            </div>
            <div v-show="totalcount>=1" >
                ...
                <div class="cart-table">
                    <div class="cart-group">
                        <!--购物列表-->
                        <div class="cart-top-items" v-for="item, index in orderdata">
                            <div class="cart-items">
                                <div class="items-choose">
                                    <span class="blue-checkbox-new" 
                                        :class="{'checkbox-on': item.selected}"
                                        @click="tabcheck(item.sku_id)"
                                    ><a></a></span>
                                </div>
                                ...
                                <div class="operation">
                                    <a class="items-delete-btn" @click="delgoods(item.sku_id)"></a>
                                </div>
                                ...
                                <div class="item-cols-num">
                                    <div class="select js-select-quantity">
                                        ...
                                        @click="subcount(item.sku_id)">-</span>
                                        <span class="num">
                                            <input type="text" v-model="item.count" style="display: inline-block;">
                                            <ul><
                                                <li>1</li>
                                                <li>2</li>
                                            </ul>
                                        </span>
                                        <span class="up" :class="{'up-disabled': item.count == item.limit_num }"
                                        @click="pluscount(item.sku_id)">+</span>
        ...
        <div class="cart-bottom-bg fix-bottom" v-show="totalcount>=1" >
            <div class="cart-bar-operation">
                <div>
                    <div class="choose-all js-choose-all">
                        <span class="blue-checkbox-new"
                        :class="{'checkbox-on': allchecked}"
                        @click="taball(allchecked)"
                        ><a></a></span>
                        全选
                    </div>
            ...
            <div class="shipping">
                <div class="shipping-box">
                    ...
                    <div class="shipping-total shipping-price">
                        <h4 class="">
                            应付总额：<span>￥</span><i >{{checkedtotalprice}}</i> 
                        </h4>
                ...
                <router-link :to="{name: 'payout'}" tag="span"
                    class="jianguo-blue-main-btn big-main-btn js-checkout"
                    :class="{' disabled-btn': checkedtotalcount <= 0}"
                ><a>现在结算</a></router-link>
            </div>
        </div>
    </div>
</div>
...
<script>
    export default {
        ...
        methods: {
            ...
            tabcheck(id){
                this.$store.commit('tabcheck',id)
            },
            taball(allchecked){
                this.$store.commit('taballcheck',allchecked)
            },
            delchecked(){
