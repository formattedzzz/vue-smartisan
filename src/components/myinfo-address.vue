...
<div class="account-address">
    <div class="gray-box clear">
        <div class="title pre-title">
            ...
            <span class="gray-blue-btn js-add-address"><a class="add" @click="showadd">添加新地址</a></span>
        </div>
        <div class="box-inner clear" v-for="add, index in addressinfo">
            <div class="address-list-item default-item" data-id="765532">
                <div class="name fn-left">
                    <div class="name-cell">{{add.name}}</div>
                </div>
                <div class="detail fn-left">
                    <div class="detail-cell">{{add.province}} {{add.city}} {{add.county}} {{add.address}}</div>
                </div>
                <div class="operation fn-right">
                    <div class="operation-cell">
                        <a class="gray-edit-btn js-edit-address" @click="showedit(index)">修改</a>
                        <span @click="deladd(index)" class="red-del-btn js-del-address"> <a>删除</a></span>
                    </div>
                </div>
                <div class="default fn-right" v-if="add.default">（默认地址）</div>
                <div class="telephone fn-right">{{add.phone}}</div>
            </div>          
        </div>
    </div>
</div>
<handleadd v-if="showaddlog" @hide="hideadd"></handleadd>
<editadd v-if="showeditlog" @hide="hideedit" :addindex="addindex"></editadd>
</div>
</template>

<script>

import handleadd from '@/components/addaddress'
